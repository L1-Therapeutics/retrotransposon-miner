...
def parse_callset(filepath: Path) -> set:
    calls = set()
    ...
    with open(filepath, "r") as f:
        for line in f:
            ...
            parts = line.split("\t")
            if len(parts) >= 5:
                # Key: (CHROM, POS, REF, ALT, MEI_FAMILY)
                calls.add((parts[0], parts[1], parts[3], parts[4]))
    return calls

def compare_parity(baseline_path: Path, candidate_path: Path) -> dict:
...
def main():
    ...
    if args.json:
        ...
        print(f"=== RTM Callset Parity Analysis ===")
