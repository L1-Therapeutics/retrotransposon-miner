...
def _load_locus_row(gold_tsv: Path, chrom: str, pos: int) -> pd.Series:
    df = pd.read_csv(gold_tsv, sep="\t", low_memory=False)
    subset = df.loc[df["chrom"].astype(str) == chrom]
    if subset.empty:
        raise ValueError(f"No rows for {chrom} in {gold_tsv}")
    hit = subset.loc[(subset["window_start"] <= pos) & (subset["window_end"] >= pos)]
    if hit.empty:
        hit = subset.assign(
            dist=(subset["window_start"] - pos).abs() + (subset["window_end"] - pos).abs()
        ).sort_values("dist").head(1)
    row = hit.iloc[0].copy()
    mei_tsv = gold_tsv.parent / "candidate_loci.mei.tsv"
    if mei_tsv.exists():
        mei_df = pd.read_csv(mei_tsv, sep="\t", low_memory=False)
        mei_hit = mei_df.loc[
            (mei_df["chrom"].astype(str) == str(row["chrom"]))
            & (mei_df["window_start"].astype(int) == int(row["window_start"]))
            & (mei_df["window_end"].astype(int) == int(row["window_end"]))
        ]
        if not mei_hit.empty:
            for col in mei_hit.columns:
                if col not in row.index or pd.isna(row.get(col)) or row.get(col) in {0, -1, "", "nan"}:
                    row[col] = mei_hit.iloc[0][col]
    return row
...
def _default_supporting_reads_detail(gold_tsv: Path) -> Path | None:
    candidate = gold_tsv.parent / "supporting_reads_detail.mei.tsv"
    return candidate if candidate.exists() else None


def _enrich_detail_with_clip_lens(detail: pd.DataFrame, gold_tsv: Path, sample: str) -> pd.DataFrame:
    ...
    if split_df.empty or "clip_len" not in split_df.columns:
        return out
    split_df = (
        split_df.sort_values("clip_len", ascending=False)
        .drop_duplicates("read_name", keep="first")
    )
    merged = out.merge(split_df[["read_name", "clip_len"]], on="read_name", how="left", suffixes=("", "_ev"))
...
def _build_read_table_for_locus(
    *,
    ...
    window_end: int,
    sample: str,
    supporting_reads_detail: Path,
    breakpoint: int | None = None,
    neighbor_bp: int = 600,
    gold_tsv: Path | None = None,
) -> pd.DataFrame:
    """Load MEI-supporting reads for the locus, including nearby windows."""
    detail = pd.read_csv(supporting_reads_detail, sep="\t")
    same_sample = (detail["chrom"].astype(str) == chrom) & (detail["sample"].astype(str) == sample)
    exact = detail.loc[
        same_sample
        & (detail["window_start"].astype(int) == window_start)
        & (detail["window_end"].astype(int) == window_end)
    ].copy()

    bp = int(breakpoint) if breakpoint is not None else (window_start + window_end) // 2
    nearby = detail.loc[
        same_sample
        & (detail["genomic_pos"].astype(int) >= bp - int(neighbor_bp))
        & (detail["genomic_pos"].astype(int) <= bp + int(neighbor_bp))
        & (
            detail.get("mate_mei_hit", False).fillna(False).astype(bool)
            | detail.get("mei_hit", False).fillna(False).astype(bool)
        )
    ].copy()

    combined = _dedupe_detail_rows(pd.concat([exact, nearby], ignore_index=True))
    if gold_tsv is not None:
        combined = _enrich_detail_with_clip_lens(combined, gold_tsv, sample)
    return combined
...
def _draw_bar_span(
    ...
    *,
    ...
    if x1 <= x0:
        return
    edgecolor = COLOR_WHITE if color == COLOR_BLACK else COLOR_BLACK
    rect = Rectangle(
        (x0, yi - height / 2.0),
        x1 - x0,
        height,
...
def _draw_read_pair(
    ...
    *,
    ...
    if is_split and remote_kind == "sr_mei":
        ...
        _draw_bar_span(ax, x0=ref_span[0], x1=ref_span[1], yi=yi, color=COLOR_BLACK, height=0.55)
        _draw_bar_span(ax, x0=mei_span[0], x1=mei_span[1], yi=yi, color=COLOR_DARK_ORANGE, height=0.55)
...
def plot_locus_architecture(
    *,
    ...
    row = _load_locus_row(gold_tsv, chrom, pos)
    ...
    fig, ax = plt.subplots(figsize=(14, max(4, len(pairs) * 0.18 + 2)))
    ...
    ax.set_title(title, color=COLOR_BLACK, fontweight="bold")
    # Prominent in-plot sample badge (disease vs control).
    ax.text(
        0.01,
        0.98,
        sample_label,
        ...
        fontweight="bold",
        color=COLOR_BLACK,
        bbox=dict(boxstyle="round,pad=0.35", facecolor=COLOR_LIGHT_ORANGE, edgecolor=COLOR_BLACK, linewidth=1.2),
        zorder=10,
    )
    ax.text(
        layout.flank_bp / 2,
    ...
    ax.text(
        layout.mei_region_end_x + layout.flank_bp / 2,
        len(pairs) + 0.55,
        f"{layout.chrom}\nright flank",
        ha="center",
        fontsize=9,
...
def main() -> None:
    ...
    parser.add_argument("--pos", type=int, required=True, help="1-based position inside locus window")
    parser.add_argument(
        "--supporting-reads-detail",
        type=Path,
        default=None,
        help="Per-read minimap detail TSV from annotate-mei-support (default: beside gold review TSV).",
    )
    parser.add_argument(
        "--sample",
        default="auto",
