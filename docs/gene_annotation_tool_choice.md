...
| Memory | none | default JVM heap → `OutOfMemoryError` building the GRCh38 interval forest; `-Xmx8g` works. `annotate-genes` passes `-noHgvs` and sets every splice-site size to 0, so snpEff does not load the reference sequence or allocate a splice interval on every exon |
