...
Use this to print a compact table for shared likely-real calls, including span,
polyA support, and TSD fields:
...
NR==1{
  ...
  print "chrom","window_start","window_end","tier","score","coherence", \
        ...
        "tumor_insertion_mei_span","tumor_poly_at_max_run","tumor_poly_at_reads", \
...
}
{
  ...
  if (t>=2 && n>=2 && s>=0.50 && coh>=0.45) {
    print $c["chrom"],$c["window_start"],$c["window_end"],$c["insertion_call_tier"],s,coh, \
          ...
          $c["tumor_insertion_mei_span"],$c["tumor_poly_at_max_run"],$c["tumor_poly_at_reads"], \
