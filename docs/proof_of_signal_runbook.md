...
Set an external working location (outside the repo) so downloaded resources and
pipeline outputs do not dirty the git checkout:
...
- MEI reference FASTA (LINE1/Alu/SVA subset), e.g.:
  - `${RTM_PUBLIC_DATA_DIR}/retrotransposon_db/dfam/dfam_human_mei_l1_alu_sva.fasta`
- Reference genome FASTA (optional but recommended for TSD sequence and breakpoint
  context annotation), e.g.:
  - `${RTM_PUBLIC_DATA_DIR}/reference/hg38/Homo_sapiens_assembly38.fasta`
- 1000G/MELT polymorphism VCF (optional; overlap + population frequency), e.g.:
  - `${RTM_PUBLIC_DATA_DIR}/polymorphism/hg38/melt/nstd144.GRCh38.variant_call.vcf.gz`
- 1000G ONT Vienna long-read SVAN polymorphism VCF (optional), e.g.:
  - `${RTM_PUBLIC_DATA_DIR}/polymorphism/hg38/long_read_1kg_ont_vienna/final-vcf.unphased.SVAN_1.3.vcf.gz`
...
```bash
ls -lh "${RTM_PUBLIC_DATA_DIR}/polymorphism/hg38/melt/nstd144.GRCh38.variant_call.vcf.gz"
```
...
```bash
bash scripts/run_proof_of_signal.sh \
  --disease-bam "${RTM_PUBLIC_DATA_DIR}/test_data/seqc2/WGS_EA_T_1.bwa.dedup.bam" \
...
- `${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/split_evidence.summary.tsv`
- `${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/candidate_loci.tsv`
- `${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/candidate_loci.mei.tsv`
...
```bash
awk -F'\t' 'NR==1{for(i=1;i<=NF;i++) c[$i]=i; next} ($c["junk_flag_count"]+0)==0 && ($c["mate_junk_flag_count"]+0)==0 && ($c["disease_mei_supported_reads"]+0)>=2 && ($c["mei_score_enrichment_ratio"]+0)>=2 && ($c["enrichment_ratio"]+0)>1 && ($c["disease_insertion_mei_span"]+0)>=30 {n++} END{print n+0}' "${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/candidate_loci.mei.tsv"
```
...
' OFS='\t' "${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/candidate_loci.mei.tsv" | sort -t$'\t' -k5,5gr
```

For chr15, replace the input path with:

`${RTM_RESULTS_DIR}/mei_step1_hg38_chr15/candidate_loci.mei.tsv`
...
' OFS='\t' "${RTM_RESULTS_DIR}/mei_step1_hg38_chr22/candidate_loci.mei.tsv" | sort -t$'\t' -k1,1 -k2,2n
```
